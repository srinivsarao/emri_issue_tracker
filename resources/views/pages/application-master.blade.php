<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ $title ?? __('Application Master') }}</h2>
    </x-slot>
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 bg-slate-50 px-5 py-5">
                    <div class="grid gap-4 md:grid-cols-[1fr_auto_auto] md:items-center">
                        <div>
                            <p class="text-sm text-slate-600">{{ $description ?? __('Manage application records and application-level settings.') }}</p>
                        </div>
                        <div class="flex items-center justify-end rounded-2xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
                            ...
                            <input id="application-search" type="text" placeholder="Search" class="ml-2 w-36 bg-transparent text-sm text-slate-900 placeholder:text-slate-400 outline-none" />
                        </div>
                        <button type="button" onclick="openApplicationMasterModal()" class="inline-flex items-center justify-center rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">Add New</button>
                    </div>
                </div>
                <div class="border-b border-slate-200 bg-slate-50 px-5 py-4">
                    <div class="flex flex-wrap items-center gap-3">
                        <button type="button" onclick="exportApplicationTable('csv')" class="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-100">Export CSV</button>
                        <button type="button" onclick="exportApplicationTable('xlsx')" class="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-100">Export XLSX</button>
                        <button type="button" onclick="exportApplicationTable('pdf')" class="rounded-lg border border-purple-200 bg-purple-50 px-3 py-2 text-sm font-semibold text-purple-700 hover:bg-purple-100">Export PDF</button>
                    </div>
                </div>
                @if(session('success') || session('error'))
                    <div class="px-5 py-4" id="application-message-container">
                        <div id="application-message" class="relative rounded-2xl px-4 py-3 text-sm font-semibold shadow-sm {{ session('success') ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700' }}">
                            <span>{{ session('success') ?? session('error') }}</span>
                            <button type="button" onclick="closeApplicationMessage()" class="absolute right-3 top-3 rounded-full bg-white/80 px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-white focus:outline-none">Close</button>
                        </div>
                    </div>
                @endif
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead class="bg-purple-100 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-purple-900">Application Name</th>
                            ...
                            <tbody class="divide-y divide-slate-200 bg-white">
                                @forelse($applications as $app)
                                    <tr>
                                        <td class="px-5 py-3 text-sm font-semibold text-slate-900">{{ $app->application_id }}</td>
                                        <td class="px-5 py-3 text-sm font-semibold text-slate-900">{{ $app->application_name }}</td>
                                        <td class="px-5 py-3 text-sm text-slate-600">{{ $app->description ?? '-' }}</td>
                                        ...
                                        <td class="px-5 py-3 text-sm">
                                            <div class="flex flex-wrap items-center gap-2">
                                                <button type="button" data-application-id="{{ $app->application_id }}" data-app-name="{{ $app->application_name }}" data-description="{{ $app->description ?? '' }}" data-status="{{ (int)$app->is_active === 1 ? 'active' : 'inactive' }}" onclick="editApplication(this.dataset)" class="rounded-lg bg-slate-900 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-slate-800">Edit</button>
        ...
        <div class="mx-auto flex max-w-2xl flex-col rounded-3xl bg-white shadow-2xl">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <div>
                    ...
                    <p class="text-sm text-slate-600">Create or update an application entry.</p>
                </div>
                <button type="button" onclick="closeApplicationMasterModal()" class="rounded-full p-2 bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-slate-900">
            ...
            <div class="space-y-4 px-5 py-5">
                <div>
                    ...
                    <input id="application_name" name="application_name" type="text" required class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400" placeholder="Enter application name" />
                </div>
                <div class="grid gap-4 md:grid-cols-2">
                    <!-- Owner and Version removed: Add new requires only name + description -->
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Description</label>
                    <textarea id="application_description" name="application_description" rows="4" required class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400" placeholder="Add description"></textarea>
                </div>
                @csrf
    ...
    <script>
        ...
        function closeApplicationMasterModal() {
            document.getElementById('application-master-modal').classList.add('hidden');
        }
        ...
        function editApplication(data) {
            ...
            document.getElementById('application_description').value = data.description || '';
            const now = currentTimestamp();
            const ua = document.getElementById('application_updated_at');
            if (ua) ua.value = now;
        ...
        function exportApplicationTable(format) {
            const params = new URLSearchParams({ format });
            window.location.href = window.location.pathname + '?' + params.toString();
        }

        function closeApplicationMessage() {
